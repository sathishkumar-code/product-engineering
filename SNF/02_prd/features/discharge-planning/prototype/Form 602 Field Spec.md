...
Complete field inventory for the Physician's Report for Community Care Facilities (State of California CDSS form LIC 602 A), the state form required when a resident is discharged to an ALF. Companion to §5.8 of the Discharge Planning PRD.
...
The form is reachable from one place: the **Form 602 card on Step 1 (Discharge Order)**, which renders only when **Discharge to = ALF**. It opens as a full-screen modal over the plan. Board & Care does not surface the card today, and Step 5's "Form 602 Signed" checkbox is shown regardless of destination — both are open questions (**OQ-20**).
...
Numbering follows the state form. "Prefill" names the source the product should use; *sample* marks a value that exists only as a demo fixture and needs a real source decision (**OQ-51**).
...
|---|---|---|
| Text fields (counted by the header pill) | 47 | 16, of which 12 are demo samples |
...
Roughly 140 inputs, of which about a tenth are prefilled. This is the single heaviest data-entry surface in the workflow and the strongest candidate for PCC-driven prefill.
...
Carried from the main PRD and still open here: **OQ-20** (which destinations require the form), **OQ-51** (auto-populate vs manual), **OQ-52** (fax destination and tracking), **OQ-53** (draft persistence and lock-after-signature), **OQ-54** (signature path).
