...
`readiness/{features,enhancements,bugs}/<slug>/test-scenarios.md` and
`test-cases.xlsx` (PM-Test-Cases and SA-Technical-Test-Cases sheets), both in
...
`build/{features,enhancements,bugs}/<slug>/qa-execution-report-<slug>.md` in
the product's GitLab `-docs` repo — see `_reference/shashi-care-doc-tree.md`.
