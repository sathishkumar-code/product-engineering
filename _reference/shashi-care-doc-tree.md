...
Instantiated from `skill-doc-tree-template.md` (Drive side; unchanged internal
shape, three parallel copies) plus `skill-gitlab-promotion-template.md` (GitLab
side; flattened by type, not mirrored from Drive). Shown together here because the
two are related by promotion, not by matching folder shapes — that mismatch is
deliberate, not a gap to close.

## Google Drive — full working structure

`{DRIVE_ROOT}/shashi-care-docs/`, Google Drive–synced (Mirror mode).
...
```
`prototype/` is retained permanently in Drive for now (Claude Design isn't yet
available to the whole team) — revisit once it is. Its eventual deletion is the
Project Manager persona's job, not Product Manager's — see the deletion policy
below.
...
## Per-slug folder shape (Drive, System Architect side) — mirrors 02_prd
...
Three separate repos, same names as the Drive folders they draw from. Each repo is
flat and grouped **by document type**, not by slug — see
`shashi-care-gitlab-binding.md` for why. No folder below ever contains anything
beyond what's listed; there is no epics-stories, test-scenarios, or review-comments
folder in GitLab at all — those never promote.
...
├── architecture/
│   └── <category>-<slug>-TD.md
├── specs/                         # developer-facing pair, promoted once approved
...
└── prototypes/                   # full prototype exports, permanent until dev
    └── <category>-<slug>/        # complete + PjM deletes (see deletion policy)
...
`specs/` deliberately breaks from Drive's author-separated storage (spec.md sits
with the PRD, tech-spec-<slug>.md sits with the TD) — GitLab is the developer-facing
side, and developers feeding Claude Code plan mode want the pair together, not
split by who wrote each half.
...
see `shashi-care-gitlab-binding.md`. Every other naming convention (Drive folders,
ClickUp Folders, filenames) stays exactly as-is; this is a one-boundary exception,
not a scheme-wide rename.

`category` is `feature` / `enhancement` / `bug` — GitLab's `prd/` and
`architecture/` are flat (no per-category subfolder), so category has to live in
the filename to stay scannable as volume grows there. Drive-side folders all
split by category now — `02_prd/{features,enhancements,bugs}/` and, as of the
`03_architecture` restructure, `03_architecture/{features,enhancements,bugs}/`
too — and keep bare, unprefixed filenames inside each `<slug>/` folder, since the
folder itself already carries the category. The `<category>-<slug>` prefix is a
GitLab-only convention now, needed there specifically because `prd/` and
...
Drive-side `SA-comments-<slug>.md`. See `skill-sa-discipline.md`.

## Prototype deletion policy
Neither deletion is automatic — Project Manager persona always confirms first, and
always leaves a one-line note behind (in `mapping-log.md` for Drive, in
`promotion-log.md` for GitLab) rather than deleting silently:
- **Drive**: delete `prototype/` the moment Epic/Story tracker items are created
  for that slug — same transaction as the mapping-log entry.
- **GitLab**: delete `prototypes/<category>-<slug>/` once development is actually
  complete (check tracker status, don't assume).

## Drive → GitLab mapping (what promotes, where it lands)

| Drive source | Trigger | GitLab target |
|---|---|---|
| `<folder>/02_prd/features/<slug>/prd-<slug>.md`, `enhancements/<slug>/enhancement-request-<slug>.md`, or `bugs/<slug>/bug-report-<slug>.md` | `status: approved` | `<repo>/prd/<category>-<slug>-PRD.md` |
| `<folder>/03_architecture/{features,enhancements,bugs}/<slug>/TD-<slug>.md` | Its own `status: approved` | `<repo>/architecture/<category>-<slug>-TD.md` |
...
| `<folder>/02_prd/.../<slug>/spec.md` | Its own `status: approved` (only after the PRD is at least approved) | `<repo>/specs/<category>-<slug>-spec.md` |
| `<folder>/03_architecture/{features,enhancements,bugs}/<slug>/tech-spec-<slug>.md` | Its own `status: approved` (only after the TD is at least approved) | `<repo>/specs/<category>-<slug>-tech-spec.md` |

Everything else in the Drive tree — epics-stories, test-scenarios, SA-comments,
handovers, the roadmap, both sync logs — stays Drive-only. Epics/Stories are
referenced live in ClickUp instead (see the ID link-back rule in
`skill-pjm-discipline.md`).

## Why the slug problem goes away (Drive side)
